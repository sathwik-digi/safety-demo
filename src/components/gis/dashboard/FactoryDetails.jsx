...
function FactoryDetails() {
  ...
  const [dynamicFactoryDetails, setDynamicFactoryDetails] = useState([]);
  ...
  return (
    <div className="mx-4 mt-6 max-w-6xl">
      {/* Factory Header Section */}
         {dynamicFactoryDetails.map((factory) => (
      <div key={factory.id} className="flex flex-col md:flex-row p-4 md:p-6 border border-gray-300 rounded-md gap-4">
        ...
        <div className="flex-1">
          <h2 className="text-[28px] md:text-[36px] font-bold text-[#000000]">{factory.name}</h2>
